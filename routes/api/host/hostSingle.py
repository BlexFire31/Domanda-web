...
@api.route("/host/single",methods=["POST"])
def hostSingle():
    ...
    if (
        ...
        if request.form.get("method")=="START":
            ...
            for question in range(1,questionsLength+1):
                isFinished=str(question)!=request.form.get("question").strip()
                runAsyncTask(
                    quizRef
                        .document("Questions")
            ...
            activeQuestion=int(verifications.get("activeQuestion")[1].get("activeQuestion"))

            runAsyncTask(
                quizRef
                    .document("Questions")
                    .collection(str(activeQuestion))#Only 1 question can be active at a time, otherwise all questions should be active
                    .document("QuestionData")
                    .update,
                {"finished":True,}
            )
