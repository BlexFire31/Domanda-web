...
@api.route("/host/get-status",methods=["POST"])
def getStatus():
    # Returns the data of all questions 


    if isInt(request.form.get("code")):
        quizRef=database.collection(request.form.get("code").strip()).document("Questions").get();

        if (
            quizRef.exists 
            # and quizRef.to_dict().get("Host")==session.get("Name")
        ):
            questionsLength=quizRef.to_dict().get("questionsLength")
            answers={}
            def hasQuestionStartedFinished(questionRef):#gets the 'started' and 'finished' params of a question and adds it to questions dictionary

                questionData=questionRef.document("QuestionData").get().to_dict();
            ...
            while len(answers) != questionsLength:pass # waiting till all the threads have finished their task
